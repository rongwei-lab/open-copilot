...
export class VisionProxyConfigStore {
	...
	getSource(): VisionProxySource | undefined {
		const currentSource = this.context.globalState.get<unknown>(VISION_PROXY_SOURCE_KEY);
		return normalizeVisionProxySource(currentSource);
	}
	...
	private async getStoredApiKey(): Promise<string | undefined> {
		const current = await this.context.secrets.get(VISION_PROXY_API_KEY_SECRET);
