...
export function streamChatCompletion({
	...
	const streamPromise =
		prepared.protocol === 'responses'
			? prepared.client.streamResponses(prepared.request, callbacks, token, {
					store: prepared.responsesStore,
					truncation: prepared.responsesTruncation,
					allowProtocolFallback: prepared.allowProtocolFallback,
				})
			: prepared.client.streamChatCompletion(prepared.request, callbacks, token);
